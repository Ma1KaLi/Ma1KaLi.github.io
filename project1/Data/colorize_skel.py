...
if __name__ == '__main__':
    ...
    for i in imname_pyramid:
        ...
        ag, disp_g = align_pyramid_simple(g, b, min_side=100, win=10)
        t1 = time.perf_counter()
        print(f"[Pyramid NCC] G→B displacement (dy,dx): {disp_g}, time: {t1 - t0:.3f}s")

        # R -> B
        t0 = time.perf_counter()
        ar, disp_r = align_pyramid_simple(r, b, min_side=100, win=10)
        t1 = time.perf_counter()
...
#------------------------Generating Self Choosing Results------------------------
if __name__ == '__main__':
    imname_pyramid  = ['Railroad.tif', 'Carving.tif', 'Gift.tif', 'Smolensk.tif']
    outname_pyramid = ['out_railroad_pyr_simple.jpg', 'out_carving_pyr_simple.jpg',
                       'out_Gift_pyr_simple.jpg', 'out_Smolensk_pyr_simple.jpg']
